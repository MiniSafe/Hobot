...
class QQBot(MessageFactory):
    ...
    def uin2qq(self,uin):
        try:
            return str(json.loads(self.getSession().urlGet('http://s.web2.qq.com/api/get_friend_uin2?tuin='+str(uin)+'&type=1&vfwebqq='+self.getSession().vfwebqq+'&t='+str(time.time()).replace('.','')+'0'))['result']['account'])
        except Exception,e:
            print e
            return ''
    def Login(self):
