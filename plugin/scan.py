...
def get_ac_ip(ip_list, msg):
    try:
        s = Nscan()
        ipPool = set(ip_list)
        return s.mPing(ipPool, msg)
    except:
    ...
    def run(self):
        while True:
            try:
                ...
                queue_task, msg = self.queue.get()
            except:
                break
            try:
                task_host, task_port = queue_task.split(":")
                data = scan_port(task_host, task_port, msg)
                if data:
                    if data is not 'NULL':
                    ...
                    if server_type: log('server', task_host, task_port, msg, server_type.strip())
            except Exception, e:
                continue
...
def scan_port(host, port, msg):
    try:
        socket.setdefaulttimeout(timeout / 2)
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((str(host), int(port)))
        log('portscan', host, port, msg)
    except Exception, e:
        return False
    try:
        data = sock.recv(512)
...
def log(scan_type, host, port, msg, info=''):
    mutex.acquire()
    try:
        if scan_type == 'portscan':
            msg.Reply("%s:%d open" % (host, int(port)))
        elif scan_type == 'server':
            msg.Reply("%s:%d is %s" % (host, int(port), str(info)))
        elif scan_type == 'active':
            msg.Reply("%s active" % (host))
    except Exception, e:
        print e
        pass
    mutex.release()
...
def read_config():
    ...
    services = ['1 tcpmux',
                ...
                '65301 pcanywhere',
                '65534 backdoor-port']
    for mark in services:
        port,name = mark.split(" ", 1)
        mark_list.append([name, port,''])
    return mark_list
...
def get_port_list(port):
    ...
    for x in port_list:
        try:
            if '-' in x:
                tmp = x.split('-', 1)
                result.extend([str(t) for t in range(int(tmp[0]), int(tmp[1]))])
            else:
                result.append(x)
        except:
            pass
...
ip = '192.168.1.132'
noping = False  # 不进行ping
port = '21,22,23,25,53,80,81,110,139,143,389,443,445,1080,1723,1433,1521,3306,3389,4899,5900,8000,8001,8080,8081,9999'
m_count = 100
...
def work(msg,info):
    if not msg.content.startswith('scan'):
        return False
    if not info.get('level',3) < 2:
        return False
    global port
    port = '21,22,23,25,53,80,81,110,139,143,389,443,445,1080,1723,1433,1521,3306,3389,4899,5900,8000,8001,8080,8081,9999'
    if msg.content.count(' ') > 0:
        ip = msg.content.split(' ', 1)[1].strip()
    else:
        return help(info)
    if ip.count(' ') > 0:
        print 'hasPort'
    ...
    if ip:
        ...
        if not noping: ip_list = get_ac_ip(ip_list, msg)
        for ip_str in ip_list:
            for port_int in port_list:
                queue.put((':'.join([ip_str, port_int]), msg))
...
def help(info):
    if not info.get('level',3) < 2:
        return False
    return "#scan ip(192.168.1.1-192.168.1.100) port(1-10,80,3000-5000) 扫描端口"
